...
function isRecipeResponse(data: unknown): data is GeneratedRecipeResponse {
  ...
  return (d.recipes as unknown[]).every(r => {
    ...
    return (
      typeof rec.recipeName      === 'string' && rec.recipeName.length > 0 &&
      typeof rec.cookingTime     === 'string' &&
      Array.isArray(rec.instructions) && (rec.instructions as unknown[]).length > 0
    );
  });
}
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onChange={e => { const v = parseFloat(e.target.value); if (!isNaN(v) && v > 0) setInputQty(v); }}
                ...
                <select
                  value={inputUnit}
                  onChange={e => { const u = e.target.value as typeof UNITS[number]; setInputUnit(u); setInputQty(getStep(u)); }}
                  ...
                  {pantry.map((item, idx) => (
                    <li key={item.id} className="group flex items-center gap-3 px-4 py-3 transition hover:bg-stone-50">
                      ...
                      <div className="flex items-center gap-1 rounded-lg bg-stone-100 px-1 py-1">
                        <button onClick={() => handleAdjust(item.id, -1)} aria-label={`Decrease ${item.name}`}
                          className="flex h-6 w-6 items-center justify-center rounded-md text-base font-bold text-stone-500 transition hover:bg-white hover:text-stone-800 hover:shadow-sm">−</button>
                        <span className="min-w-[4.5rem] text-center text-xs font-semibold tabular-nums text-stone-700">
                          {formatQty(item.quantity, item.unit)}
                        </span>
                        <button onClick={() => handleAdjust(item.id, 1)} aria-label={`Increase ${item.name}`}
                          className="flex h-6 w-6 items-center justify-center rounded-md text-base font-bold text-stone-500 transition hover:bg-white hover:text-stone-800 hover:shadow-sm">+</button>
                      </div>
                      <button onClick={() => handleRemove(item.id)} aria-label={`Remove ${item.name}`}
                        className="flex h-7 w-7 items-center justify-center rounded-lg text-xs text-stone-300 opacity-0 transition-all hover:bg-red-50 hover:text-red-500 group-hover:opacity-100">✕</button>
                    </li>
                  ))}
                </ul>
              )}
            ...
            <div className="rounded-2xl border border-stone-200 bg-white p-7 shadow-sm">
              <div className="flex flex-col gap-8">
                ...
                <div>
                  ...
                  <input id="prep-time" type="range" min={10} max={120} step={5}
                    value={config.prepTime} onChange={e => setPrepTime(Number(e.target.value))} />
                  ...
                  <div className="mt-3 flex items-center gap-3">
                    ...
                    <div className="flex flex-1 items-center justify-center gap-1">
                      {Array.from({ length: Math.min(servingsCount, 8) }).map((_, i) => (
                        <span key={i} className="text-base" title={`${i + 1} person`}>🧑</span>
                      ))}
                      {servingsCount > 8 && (
                        <span className="text-xs font-semibold text-stone-500">+{servingsCount - 8}</span>
                      )}
                    </div>
                    <button
